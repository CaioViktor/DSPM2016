...
//        dao.update(note);
        ...
        ContentValues contenteVaulues = new ContentValues();
        contenteVaulues.put(NotesProvider.TITLE,note.getTitle());
        contenteVaulues.put(NotesProvider.CONTENT,note.getContent());
        contenteVaulues.put(NotesProvider.DATA,note.getData());
        getContentResolver().update(NotesProvider.CONTENT_URI,contenteVaulues,"id = ?",args);
...
//        dao.delete(note.getId());
        String[] args = {note.getId()+""};
        getContentResolver().delete(NotesProvider.CONTENT_URI,"id = ?",args);
        setResult(RESULT_OK, null);
        finish();
    }
}